...
- maintain deque which contains elms in decreasing order so that ans for each window is at fornt of deque. dq will store elms in decreasing order(maxm to min at end)
- process remn window doing the same
*/
...
void solve(vector<int>&nums,int k){
    ...
    for(int i = k;i<nums.size();i++){
        ...
        if(!dq.empty() && i-dq.front()+1 > k){  // when window becomes greater than k, we have to remove one elm from start (condition same as j-i+1)
